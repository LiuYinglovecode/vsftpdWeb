	...
	*/	 
	...
	public function delete_user($id)
	{
		$q="SELECT value FROM settings WHERE name = 'user_path' ;";
    		$user_path = $this->db->query($q)->row();
    		$user_path = $user_path->value;
		$q="SELECT username FROM accounts WHERE id = $id ;";
    		$username = $this->db->query($q)->row();
    		$username = $username->username;

		if (!empty($username) && $username!="" && !preg_match('/\s/',$username)) {
			try {
		    		exec("sudo rm -rf /".$user_path.$username);
                	} catch (Exception $e) {
                		try {
			    		exec("sudo rm -rf /etc/vsftpd/vusers/".$username);
				} catch (Exception $i) {}
                	}
                }
